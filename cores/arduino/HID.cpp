...
static const struct device *const _hid_dev =
    DEVICE_DT_GET(DT_INST(0, zephyr_hid_device));
...
HID_::HID_()
    : _descriptors(nullptr), _descriptorSize(0),
      _registered(false), _active(false), _ifaceReady(false)
{
}
...
int HID_::SendReport(uint8_t id, const void *data, int len)
{
    ...
    if (id > 0) {
        ...
        return hid_device_submit_report(_hid_dev,
                                        (uint16_t)(len + 1),
                                        _report_buf);
    } else {
